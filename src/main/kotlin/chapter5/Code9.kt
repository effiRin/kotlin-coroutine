...
class `Code9` {
    ...
    fun `Code5-9`() = runBlocking {
         ...
         *
         * withContext 함수가 호출되면 실행 중인 코루틴의 실행 환경이 withContext 함수의 context 인자값으로 변경돼 실행되며，이를 컨텍스트 스위칭 ContextSwitching이라고 부른다.
         * withContext(Dispatchers.IO)가 호출되면 해당 코루틴은 Dispatchers.IO의 작업 대기열로 이동한 후 Dispatchers.IO가 사용할 수 있는 스레드 중 하나로 보내져 실행된다.
         */
    }
    ...
    fun `Code5-10`() = runBlocking {
        ...
        /***
         * 코드의 실행 결과를 보면 async 블록을 실행하는 코루틴은 coroutine#2로 runBlocking 블록을 실행하는 coroutine^과 다른 것을 볼 수 있다.
         * 즉, async- await 쌍을 사용하면 새로운 코루틴을 만들지만 await 함수가 호출돼 순차 처리 가 돼 동기적으로 실행되는 것이다
         */
    }

    /***
     * 정리하자면,
     * withContext를 호출하면 코루틴이 유지된 채로 코루틴을 실행하는 실행 스레드만 변경되기 때문에 동기적으로 실행되는 것이고,
